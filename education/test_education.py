...
class LessonAndSubscriptionTests(APITestCase):
    ...
    def test_create_lesson_owner(self):
        ...
        response = self.client.post("/lessons/create/", {"title": "Новый урок", "course": self.course.id})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)


    def test_create_lesson_moderator(self):
        self.client.force_authenticate(user=self.moderator)
        response = self.client.post("/lessons/create/", {"title": "Урок модератора", "course": self.course.id})
    ...
    def test_update_lesson_owner(self):
        self.client.force_authenticate(user=self.owner)
        response = self.client.patch(f"/lessons/{self.lesson.id}/update/", {"title": "Обновлено владельцем"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)


    def test_update_lesson_moderator(self):
        self.client.force_authenticate(user=self.moderator)
        response = self.client.patch(f"/lessons/{self.lesson.id}/update/", {"title": "Обновлено модератором"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_subscribe_to_course(self):
        self.client.force_authenticate(user=self.other_user)
        response = self.client.post("/courses/subscribe/", {"course_id": self.course.id})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_unsubscribe_from_course(self):
        self.client.force_authenticate(user=self.other_user)
        self.client.post("/courses/subscribe/", {"course_id": self.course.id})
        response = self.client.post("/courses/subscribe/", {"course_id": self.course.id})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_subscribe_without_course_id(self):
        self.client.force_authenticate(user=self.other_user)
        response = self.client.post("/courses/subscribe/", {})
